...
Deterministic models (MSM, HMM, HMM_Uni) produce reproducible results under identical configuration. LSTM and Transformer vary slightly between runs due to non-deterministic training (random weight initialization, batch shuffling). These deviations cascade into backtesting, SORR, and the Monte Carlo simulation.
