...
| [src/backtest/seed_sensitivity.py](../src/backtest/seed_sensitivity.py) | Retraining-stability quantification: per-model seed CV of the OOS metrics on the production config (route `POST /models/seed-sensitivity`) |
...
| [assets/seed_sensitivity.md](../assets/seed_sensitivity.md) | Retraining-stability report: per-model seed CV of the OOS metrics (route `POST /models/seed-sensitivity`, `src/backtest/seed_sensitivity.py`) |
...
| File | Description |
|-------|-------------|
| [assets/h1_drawdown_test.md](../assets/h1_drawdown_test.md) | H1: drawdown reduction (significant?), one block per scenario; the `Headline` column marks `evaluation.extended.hypothesis_scenario` |
| [assets/mcs_h1_mdd_forest.png](../assets/mcs_h1_mdd_forest.png) | H1: forest plot H1/MDD, one panel per scenario |
| [assets/h2_transformer_test.md](../assets/h2_transformer_test.md) | H2: Transformer superiority (significant?), one block per scenario |
| [assets/mcs_h2_final_capital_forest.png](../assets/mcs_h2_final_capital_forest.png) | H2: forest plot H2/terminal capital, one panel per scenario |

> **Reading H1.** The MCS MaxDD is measured on the decumulation capital path,
> i.e. after withdrawals, so it is not a pure market-risk measure: a depleted
> path is -100 % by construction. At the overstretched (`Aggressive`) rate the
> mechanical drain dominates and H1 behaves like a survival test, which is why
> its verdict can invert relative to the historical backtest, where the same
> models reduce drawdown substantially. Report the scenarios together.
...
| [assets/mcs_depletion_rate_forest.png](../assets/mcs_depletion_rate_forest.png) | Forest plot, depletion rate with Wilson CI, one panel per scenario
