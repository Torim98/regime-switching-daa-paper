...
### `POST /models/seed-sensitivity`
- **Parameters**: `seeds` (query, int, default `5`), `models` (query, string: `all` | `dl`, default `all`)
- **Description**: Quantifies the retraining stability of the production config: re-runs each model on the config.yaml hyperparameters and the full walk-forward fold set, varying only its random source (EM initialization for HMM/HMM_Uni, global RNG for LSTM/Transformer; MSM is the deterministic zero-variance control) and reports mean/std/CV of the pooled OOS metrics. `models=dl` restricts to LSTM and Transformer for a faster check. Re-trains the DL models per seed on the GPU (slow). Writes `assets/seed_sensitivity.md`, rendered on the Models dashboard page, and returns the headline CV per model. Backed by `src/backtest/seed_sensitivity.py` (also runnable as a CLI). Motivates the multi-start EM (`models.hmm.n_init`) and the DL seed ensemble (`walk_forward.dl_ensemble_size`).
